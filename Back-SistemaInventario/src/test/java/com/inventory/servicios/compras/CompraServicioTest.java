...
public class CompraServicioTest {
    ...
    void testConsultarHistorial() throws Exception {
        ...
        when(purchaseDetailRepository.obtenerHistoricoCompras(any(), any(), any(), any(), any(), any(), any())).thenReturn(mockPage);

        Page<CompraHistoricoRespuestaDTO> historial = compraServicio.obtenerHistoricoCompras(
                1L, null, null, 1L, LocalDateTime.now().minusDays(30), LocalDateTime.now(), 0, 10
        );
