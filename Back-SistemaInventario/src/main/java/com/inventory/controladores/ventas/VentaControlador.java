...
public class VentaControlador {
    ...
    public ResponseEntity<MensajeDTO<Object>> createSale(@Valid @RequestBody VentaCrearDTO dto) {
        ...
        return ResponseEntity.ok(new MensajeDTO<>(false, saleService.createSale(dto)));
    }
    ...
    public ResponseEntity<MensajeDTO<Object>> getSalesByDateRange(
        ...
        return ResponseEntity
                .ok(new MensajeDTO<>(false, saleService.getSalesByDateRange(start, end, pagina, porPagina)));
    }

    @GetMapping("/comprobante/{id}")
    public ResponseEntity<MensajeDTO<String>> getReceipt(@PathVariable Long id) {
        return ResponseEntity.ok(new MensajeDTO<>(false, receiptService.obtenerBase64(id)));
    }
